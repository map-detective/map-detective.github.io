...
export default {
    ...
    actions: {
        ...
        async signInHost() {
            ...
            if (
                allowed.length > 0 &&
                !isAllowedAccount(result.user?.email, allowed)
            ) {
                // ログイン状態を残すと、次回以降も許可されないまま進もうとしてしまう
                await firebase.auth().signOut();

                const error = new Error(
                    'This account is not allowed to host a room.'
                );
        ...
        openDialogRoom({ commit }) {
            commit(MutationTypes.SETTINGS_SET_MODE_DIALOG_ROOM);
        ...
        setPlayerName({ commit, state }, raw) {
            ...
            let invalid = false;
            if (
                norm.length < PLAYER_NAME_MIN ||
                norm.length > PLAYER_NAME_MAX
            ) {
                invalid = true;
            ...
            commit(MutationTypes.SETTINGS_SET_PLAYER_NAME, {
                playerName: norm,
                invalid,
            });
        },
        ...
        startGameMultiplayer({ state, rootState, dispatch }, gameParams) {
            ...
            const fromRef =
                state.room && typeof state.room.key === 'string'
                    ? state.room.key
                    : '';
            const fromRoute =
